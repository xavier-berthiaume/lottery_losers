...
class Root():
    ...
    def createDrawFrame(self):
        ...
        draw_frame.grid(row = 0, column = 0, padx = self.generic_pad_x, pady = self.generic_pad_y)
        count = 1
        ...
        draw_button = Button(draw_frame, text = "Start Drawing Tickets", command = lambda: self.draw_engine.startDrawing())
        draw_button.grid(row = 100, column = 0, columnspan = 2, padx = self.generic_pad_x, pady = self.generic_pad_y, sticky = E+W)
    ...
    def createInfoFrame(self):
        ...
        info_frame.grid(row = 0, column = 1, padx = self.generic_pad_x, pady = self.generic_pad_y, sticky = E+W+N+S)

        self.match_labels = []
        for x in range(1, 6):
            self.match_labels.append(Label(info_frame, text = f"{x} match: "))
