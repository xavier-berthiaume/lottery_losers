...
class Root():
    ...
    def createDrawFrame(self):
        ...
        for x in range(0,13,2):
            if count == 7:
                draw_label = Label(draw_frame, text = f"Bonus #")
        ...
        draw_button = Button(draw_frame, text = "Start Drawing", command = lambda: self.runner.draw_engine.startDrawing(), height = button_height_generic)
        ...
        save_button = Button(draw_frame, text = 'Save Run', command = lambda: self.runner.saveRun())
        save_button.grid(row = 15, column  = 0, columnspan = 2, padx = self.generic_pad_x, pady = self.generic_pad_y, sticky = E+W)
    ...
    def createInfoFrame(self):
        ...
        self.match_labels.append(Label(info_frame, text = f"2 match /w Bonus: 0"))
        self.match_labels[-1].grid(row = 7, column = 0, padx = self.generic_pad_x, pady = self.generic_pad_y, sticky = W)

        self.match_labels.append(Label(info_frame, text = f"5 match /w Bonus: 0"))
        self.match_labels[-1].grid(row = 8, column = 0, padx = self.generic_pad_x, pady = self.generic_pad_y, sticky = W)

        self.draws_label = Label(info_frame, text = "Total draws: 0")
        self.draws_label.grid(row = 9, column = 0, padx = self.generic_pad_x, pady = self.generic_pad_y, sticky = E+W)

        self.cost_label = Label(info_frame, text = "Total ticket cost: $0")
        self.cost_label.grid(row = 10, column = 0, padx = self.generic_pad_x, pady = self.generic_pad_y, sticky = E+W)

        self.winnings_label = Label(info_frame, text = "Winnings: $0")
        self.winnings_label.grid(row = 11, column = 0, padx = self.generic_pad_x, pady = self.generic_pad_y, sticky = E+W)
